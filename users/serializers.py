...
class UserSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ["id", "login", "password", "phone_number", "birth_date", "created_at", "updated_at"]
        extra_kwargs = {
    ...
    def validate_login(self, value):
        ...
        if domain not in allowed_domains:
            raise serializers.ValidationError(f'Разрешены только домены: {", ".join(allowed_domains)}')
        return value
